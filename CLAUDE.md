...
- **Compact at 200k.** When the context window reaches about 200k tokens, run `/compact` before starting the next step, not in the middle of one. Land or hand off whatever is in flight first: a compaction mid-edit loses the detail the edit depended on. Write the handoff file below when the task will outlive the compaction.
