 ...
 *   calls setupGmailWatch with PUBSUB_TOPIC, then calls pollGmail, and
 *   returns { watch_renewed: true } in the response body.
 *
 * All external I/O is mocked — no real DB, Gmail API, or Supabase calls.
 ...
 * implementation, while this file mocks pollGmail at the route-handler level.
 */
...
vi.mock("@/server/email/gmail/poll-state", () => ({
  ...
  // These are not called by the cron route directly but are included for
  // completeness in case the module is transitively imported.
...
describe("AC9 (integration) — cron route watch-renewal branch", () => {
  beforeEach(() => {
    ...
    mockPollGmail.mockResolvedValue(POLL_RESULT);
  ...
  it("AC9: renews watch and polls when expiration is within 24h threshold", async () => {
    ...
    expect(mockSetupGmailWatch).toHaveBeenCalledWith(PUBSUB_TOPIC_VALUE);

    // pollGmail must also have been called (renewal does not skip polling).
    expect(mockPollGmail).toHaveBeenCalledOnce();
