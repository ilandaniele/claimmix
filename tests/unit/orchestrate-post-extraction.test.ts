...
// ── Test suite: full_name igual al nombre del sobre ──────────────────────────

/*
 * Caso real 9f7e8c3a: el `From` decía «Ilan Daniele <…>», el cuerpo no
 ...
 * `full_name` de confianza media que repite el nombre visible del sobre ya no
 * se pregunta — ver `esNombreDelSobre`.
 */
describe("orchestratePostExtraction — full_name igual al nombre del sobre", () => {
  const SOBRE_CON_NOMBRE = "Ilan Daniele <ilan.daniele@example.com>";

  function claimConNombre(valor: string) {
    return extractEmailClaimMock({
      fields_pending_confirmation: ["full_name"],
      fields: [
        ...extractEmailClaimMock().fields.filter((f) => f.field_key !== "full_name"),
        { field_key: "full_name", field_value: valor, confidence: 0.72, source: "ai" as const },
      ],
    });
  }

  it("el mismo nombre del sobre se promueve: no se pregunta", async () => {
    await orchestratePostExtraction(
      CASE_ID,
      TENANT_ID,
      { extractedClaim: claimConNombre("Ilan Daniele"), senderEmail: SOBRE_CON_NOMBRE },
      NO_MATCHES
    );
  ...
  it("el mismo nombre con acentos, mayúsculas y espacios distintos también se promueve", async () => {
    await orchestratePostExtraction(
      CASE_ID,
      TENANT_ID,
      { extractedClaim: claimConNombre("  ILAN   DÁNIELE  "), senderEmail: SOBRE_CON_NOMBRE },
      NO_MATCHES
    );
  ...
  it("un nombre distinto al del sobre sigue pendiente", async () => {
    await orchestratePostExtraction(
      CASE_ID,
      TENANT_ID,
      { extractedClaim: claimConNombre("Roberto Paz"), senderEmail: SOBRE_CON_NOMBRE },
      NO_MATCHES
    );
  ...
  it("sin nombre visible en el sobre (dirección pelada) sigue pendiente", async () => {
    await orchestratePostExtraction(
      CASE_ID,
      TENANT_ID,
      { extractedClaim: claimConNombre("Ilan Daniele"), senderEmail: SENDER_EMAIL },
      NO_MATCHES
    );
